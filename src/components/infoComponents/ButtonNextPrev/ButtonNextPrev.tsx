...
export function ButtonNextPrev() {
  ...
  const prev = pokedex.pokemon_entries.find(
    (pokemon: PokemonEntry) =>
      pokemon.entry_number ===
      pokemonSpecies.pokedex_numbers[0].entry_number - 1
  );
  const next = pokedex.pokemon_entries.find(
    (pokemon: PokemonEntry) =>
      pokemon.entry_number ===
      pokemonSpecies.pokedex_numbers[0].entry_number + 1
  );
    ...
    <ButtonContainer color="lightgray">
      {pokemonSpecies.pokedex_numbers[0].entry_number > 1 ? (
        <PrevButton onClick={() => handleClick(prev.entry_number)}>
          <PokeImg
            src={
              prev.entry_number > 649
                ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${prev.entry_number}.png`
                : `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${prev.entry_number}.svg`
            }
            alt={prev.pokemon_species.name}
          />
      ...
      {pokemonSpecies.pokedex_numbers[0].entry_number < 898 ? (
        <NextButton onClick={() => handleClick(next.entry_number)}>
          <PokeImg
            src={
              next.entry_number > 649
                ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${next.entry_number}.png`
                : `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${next.entry_number}.svg`
            }
            alt={next.pokemon_species.name}
          />
